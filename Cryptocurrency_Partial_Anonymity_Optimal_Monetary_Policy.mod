...
mcm   ${\hat{mc}_m}$       (long_name='marginal cost of traditional money sector')
mcc   ${\hat{mc}_c}$       (long_name='marginal cost of cryptocurrency sector')
 ymt  ${y_m}$              (long_name='traditional money sectoral output')
 yct  ${y_c}$             (long_name='cryptocurrency sectoral output')
 ...
 w_hat   ${\hat{w}}$       (long_name='aggregated real wage') 
pi_agg   ${\hat{\pi}^{agg}}$ (long_name='aggregated price level')
...
er1      ${er_1}$           (long_name='first argument of excess return')
er2      ${er_2}$           (long_name='second argument of excess return')
ymt_gap  ${\hat{y}_m}$      (long_name='traditional money sectoral output gap')
yct_gap  ${\hat{y}_c}$      (long_name='cryptocurrency sectoral output gap')
;
...
parameters nu_d nu_ci zeta_cb_para rhoc alpha sigma phi niz betta theta1 theta2 eta  gamma9  ni rhoa  rhoz rhoe varphi rho ksiphi rhoksi rhonit omega2 omega3 gamma4 gamma8 gamma2 gamma3 gamma6 gamma7 rhor rhoy rhopi rhomu gamma1 gamma5 zeta rhom rhoed zetaa;
...
//(6)&(7)NKPC w.r.t cryptocurrency sector partial anonymity
pji = betta*pji(+1)+mcc-((1-theta2)*(1-betta*theta2)/theta2)*ni*s;
mcc=((1-theta2)*(1-theta2*betta)/theta2)*((1-alpha)/(1-alpha+alpha*eta))*(sigma)*(x-zeta*CC-zeta*hi-omega3*(hi+CC-ejd)-omega2*(M-ej))+((1-theta2)*(1-betta*theta2)/theta2)*((1-alpha)/(1-alpha+alpha*eta))*((alpha+phi)/(1-alpha))*(yct);

//(8)Aggregate inflation
pi_agg=(1-ni)*pji+(ni)*pi;

//(9)&(10)&(11)Relative price equations
...
//(30)&(31)&(32)&(33) sectoral outputs and output gaps
ymt=c-eta*p_m_hat;
yct=c-eta*p_c_hat;
ymt_gap=ymt-a;
yct_gap=yct-a;
...
//(36)Monetary authority clearing:
d=M*nu_d;

//(37)Investor constraint
div=xx-w_hat-n;
...
estimation(datafile=ccestrrnew,diffuse_filter, mode_check,mode_compute=6,mh_replic=10000,mh_nblocks=2, mh_jscale=0.5, mh_drop=0.2) x pi rc M CC pji; 
//stoch_simul(order=1, irf=40)x pi rc M CC pji s ymt yct c mcm mcc;
stoch_simul(order=1, irf=40)ymt ymt_gap pi pji yct_gap;
...
CH_pos=strmatch('yct_gap',var_list_ ,'exact');
...
shocks;
    ...
    for ii=1:length(phi_pi_vec)
        ...
        if ~info(1)
          ...
          L(ii)=0.5*((par.sigma)*(variance.y_gap(ii))+((par.varphi+par.alpha)/(1-par.alpha)*variance.CH(ii))+par.eta/par.lambda*variance.pi(ii)+par.eta/par.lambda2*(variance.pji(ii)))/100;
      end
    end
    %Print result
